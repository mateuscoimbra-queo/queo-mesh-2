...
def build_payload(node_id, short_id, target, user_message):
    ...
    - last_sent_at/attempts: controle de retry;
    - acked: status de confirmação;
    - path: caminho inicial da mensagem.
    """
...
def enqueue_message(
    ...
    runtime_flags["RELAY_CHECK"] = True
...
def create_input_loop(
    ...
    def input_loop():
        while not stop_event.is_set():
            ...
            #     runtime_flags["hiden_log"] = not runtime_flags["hiden_log"]
            #     print(f"{YELLOW}[INPUT]{RESET} hiden_log = {runtime_flags['hiden_log']}")
            #     continue
...
def build_send_payload(updated_payload, node_id, short_id):
    """
...
def sender_loop(
    ...
    while not stop_event.is_set():
        ...
        for msg in pending_messages:
            ...
            payload_to_send = build_send_payload(
                updated_payload,
                node_id,
                short_id
            )
...
def print_outbox_status_if_needed(
    ...
    if runtime_flags["RELAY_CHECK"]:
        runtime_flags["RELAY_CHECK"] = False
