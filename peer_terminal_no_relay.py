"""
Peer Zenoh terminal sem relay manual.
...
- recebe comandos;
- identifica mensagens duplicadas apenas para controle local de recebimento;
- envia ACK para a origem da mensagem;
- mantém logs importantes de depuração.

A aplicação não faz relay manual.
A entrega, propagação e roteamento entre peers ficam sob responsabilidade do Zenoh.
"""
...
def build_zenoh_config(listen_endpoint, neighbor_endpoints, gossip_enabled):
    ...
    - endpoints conhecidos para conexão inicial;
    - multicast desabilitado;
    - gossip configurável;
    - roteamento peer em modo linkstate.

    A aplicação não usa esses endpoints para fazer relay manual.
    Eles servem apenas para a infraestrutura Zenoh formar a malha.
    """
...
def send_ack(zenoh_session, node_id, short_id, msg_id, origin_id):
    ...
    print(
        f"{BLUE}[ACK-SENT]{RESET} "
        f"{ack_key}: {json.dumps(ack_payload, ensure_ascii=False)}"
    )
...
def register_processed_message(
    ...
    """
    Registra localmente uma mensagem recebida.

    Este cache não é usado para controlar relay ou repasse.
    Ele serve apenas para evitar processamento local repetido caso a mesma
    mensagem chegue mais de uma vez ao nó terminal.
    """
...
def create_command_handler(
    ...
    Este handler apenas recebe, registra localmente e responde com ACK.
    Ele não repassa comandos para outros peers.
    """
    ...
    def on_command(sample):
        ...
        if is_duplicate:
            print(f"{YELLOW}[CMD-DUPLICADO]{RESET} mensagem repetida chegou: {msg_id}")
...
def cleanup_processed_messages(processed_messages, processed_lock):
    """
    Remove mensagens antigas do cache local de processadas.
    """
...
def liveness_checker(
    ...
    Esta verificação não interfere no roteamento.
    É apenas diagnóstico de liveness.
    """
        ...
        with peers_lock:
            for peer, info in peers_status.items():
                ...
                if elapsed > LIVENESS_TIMEOUT:
                    ...
                    print(
                        f"{GREEN}[LIVENESS]{RESET} "
                        f"A conexão com {peer} foi perdida "
                        f"(timeout de {int(elapsed)}s)"
                    )
            ...
            if peers_status:
                ...
                for peer, info in sorted(peers_status.items()):
                    if info["status"] == "ONLINE":
                        ...
                        connect_status = f"{RED}OFFLINE{RESET}"
...
def print_message_status_if_needed(
    ...
    if runtime_flags["MESSAGE_CHECK"]:
        ...
        with processed_lock:
            ...
            print("\n--------- STATUS DAS MENSAGENS ---------")
            print(f"processadas no cache local: {total_msgs}")
...
def start_background_threads(
    ...
    threading.Thread(
        ...
        args=(peers_status, peers_lock, runtime_flags),
        daemon=True
    ).start()
...
def run_node(cfg):
    ...
    gossip_enabled = cfg.get("gossip_enabled", False)
    ...
    # Cache local de mensagens já recebidas.
    # Não é usado para controlar relay ou repasse.
